               ...
               <div className="py-1">
                  {options.map((option) => (
                     <button
                        ...
                        className={`w-full text-left px-4 py-2.5 text-xs font-medium flex items-center justify-between transition-colors ${value === option.value
                     ...
                     {logs.map((log, i) => (
                        <tr key={i} className="hover:bg-white/[0.02]">
                           ...
                           <td className="p-3 text-right">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${log.status === 'Success' ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger'
                                 }`}>
                                 {log.status}
                              </span>
                           </td>
            ...
            <div className="space-y-3 mb-6">
               {['Starter', 'Pro', 'Enterprise'].map(plan => (
                  <div
                     ...
                     className={`p-4 rounded-xl border cursor-pointer transition-all flex justify-between items-center ${selectedPlan === plan
         ...
         <div className="flex flex-col md:flex-row md:items-center gap-4 border-b border-white/5 pb-6">
            ...
            <div>
               <h2 className="text-2xl font-bold text-slate-100 flex items-center gap-3">
                  {tenant.name}
                  <span className={`text-xs px-2 py-0.5 rounded border font-medium uppercase tracking-wide ${tenant.status === 'Active' ? 'border-success/30 text-success bg-success/5' : 'border-danger/30 text-danger bg-danger/5'
                     }`}>{tenant.status}</span>
               </h2>
         ...
         <div className="flex gap-1 bg-white/5 p-1 rounded-lg w-fit overflow-x-auto">
            {['Overview', 'Settings', 'API Keys', 'Billing'].map(tab => (
               <button
                  ...
                  className={`px-4 py-2 rounded-md text-sm font-medium transition-all whitespace-nowrap ${activeTab === tab ? 'bg-surface shadow text-white' : 'text-slate-400 hover:text-slate-200'
                     }`}
               >
                  {tab}
               </button>
...
const Tenants: React.FC = () => {
   ...
   const handleCreateSubmit = async (newTenant: Tenant) => {
      if (!AccessControlService.canCreateTenant(user)) {
         notify("Unauthorized: Only Super Admins can create tenants.", "error");
         return;
      }
                        ...
                        >
                           ...
                           <div className="flex justify-between items-start mb-6">
                              ...
                              <div className="flex items-center gap-3">
                                 <div className={`px-2 py-1 rounded text-xs font-medium border ${tenant.status === 'Active'
                           ...
                           <tbody className="divide-y divide-white/5">
                              {paginatedTenants.map(tenant => (
                                 <tr key={tenant.id} className="hover:bg-white/[0.02] transition-colors group">
                                    ...
                                    <td className="p-4">
                                       <span className={`text-xs font-medium px-2 py-1 rounded border ${tenant.status === 'Active'
