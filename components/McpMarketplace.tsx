...
import { McpService, AccessControlService } from '../services';
...
const McpMarketplace: React.FC = () => {
   const { mcpProviders, setMcpProviders, user } = useGlobal();
   const canManage = user ? AccessControlService.canManageMcpProviders(user) : false;
                  ...
                  return (
                     <div key={provider.id} className="bg-surface border border-white/5 rounded-2xl p-6 hover:border-primary/30 transition-all group flex flex-col h-full relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-4">
                           ...
                           <div className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border flex items-center gap-1 ${provider.status === 'Installed' ? 'bg-success/10 text-success border-success/20' :
                        ...
                        <div className="mt-auto pt-4 border-t border-white/5">
                           {provider.status === 'Installed' ? (
                              <button
                                 ...
                                 disabled={!canManage}
                                 className="w-full py-2.5 bg-surfaceHighlight border border-white/10 rounded-xl text-slate-300 text-sm font-medium hover:text-white hover:bg-white/10 transition-colors flex items-center justify-center gap-2 group-hover:border-primary/30 disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                              ...
                              <button
                                 ...
                                 className="w-full py-2.5 bg-warning/10 border border-warning/20 rounded-xl text-warning text-sm font-bold hover:bg-warning/20 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                              ...
                              <button
                                 ...
                                 disabled={isProcessing || !canManage}
                                 className="w-full py-2.5 bg-primary text-white rounded-xl text-sm font-bold hover:bg-primaryHover shadow-neon transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                              >
